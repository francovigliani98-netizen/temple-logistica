  ...
  dd.innerHTML = filtrados.slice(0, 20).map(p => `
    <div onclick="seleccionarProducto(${p.sku})"
      ...
      <div style="font-size:11px;color:var(--text3);margin-top:2px">
        SKU ${p.sku} · $${Math.round(p.precio_unidad||0).toLocaleString('es-AR')}/unidad · ${p.unidades_por_bulto} u/bulto
      </div>
    </div>`).join('');
  dd.style.display = 'block';
}
...
function seleccionarProducto(sku) {
  ...
  prod._precioUnidad = Math.round(prod.precio_unidad || 0);
  document.getElementById('real-prod-precio-base').textContent =
    `SKU ${prod.sku} · Precio lista: $${prod._precioUnidad.toLocaleString('es-AR')}/unidad`;
...
function actualizarLineaPreview() {
  ...
  const unidades = parseInt(document.getElementById('real-prod-bultos')?.value) || 1;
  const desc     = parseFloat(document.getElementById('real-prod-descuento')?.value) || 0;
  const precioBase  = prodSeleccionado._precioUnidad || Math.round(prodSeleccionado.precio_unidad || 0);
  const precioFinal = precioBase * (1 - desc / 100);
  const total = precioFinal * unidades;
  const prev = document.getElementById('real-linea-preview');
  if (prev) {
    prev.innerHTML = desc > 0
      ? `${unidades} u × $${Math.round(precioFinal).toLocaleString('es-AR')} (${desc}% dto.) = <strong>$${Math.round(total).toLocaleString('es-AR')}</strong>`
      : `${unidades} u × $${Math.round(precioBase).toLocaleString('es-AR')} = <strong>$${Math.round(total).toLocaleString('es-AR')}</strong>`;
  }
}
...
function agregarLineaPedido() {
  ...
  const precioBase  = prodSeleccionado._precioUnidad || Math.round(prodSeleccionado.precio_unidad || 0);
  const precioFinal = precioBase * (1 - desc / 100);
  const unidadesPorBulto = prodSeleccionado.unidades_por_bulto || 1;
  const bultos = unidades / unidadesPorBulto; // cajas para el cálculo de flete

  // Si ya existe ese SKU con mismo descuento, sumar unidades
  const existing = pedidoLineas.find(l => l.sku === prodSeleccionado.sku && l.descuento === desc);
  if (existing) {
    existing.unidades += unidades;
    existing.bultos = existing.unidades / unidadesPorBulto;
    existing.total = existing.precioFinal * existing.unidades;
  } else {
    pedidoLineas.push({
  ...
  list.innerHTML = pedidoLineas.map((l, i) => `
    <div style="display:flex;align-items:flex-start;gap:8px;padding:8px 10px;background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius);margin-bottom:6px">
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:11px;color:var(--text3);margin-top:2px">
          ${l.unidades} u${l.unidades>1?'':''} · ${l.descuento>0?l.descuento+'% dto. · ':''}$${Math.round(l.precioFinal).toLocaleString('es-AR')}/u
        </div>
      </div>
...
function calcReal() {
  ...
  const cajas = Math.ceil(pedidoLineas.reduce((s, l) => s + l.bultos, 0));
      ...
      ${pedidoLineas.map(l => {
        ...
        return `<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid var(--border);font-size:12px">
          ...
          <div style="display:flex;gap:8px;align-items:center;flex-shrink:0;margin-left:8px">
            <span style="color:var(--text3)">${l.unidades} u · ${peso(l.total)}</span>
