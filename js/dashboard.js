...
function renderChartsCostos(){
  ...
  const prodPct={};
  // ---- PARETO: VOLUMEN Y GASTO LOGÍSTICO POR PRODUCTO ----
  // Datos de volumen desde pedido_productos (mixData)
  const paretoVolMap={};
  mixData.forEach(r=>{
  ...
  // Datos de gasto logístico: distribuir el total del pedido entre los productos del pedido
  // Usamos el mixData para saber qué productos había en cada pedido
  const paretoGastoMap={};
  // Agrupar pedidos por pid con su total logístico
  const pedidoTotales={};
  rows.forEach(r=>{if(r.pid&&r.total)pedidoTotales[r.pid]=r.total;});
  // Para cada línea de pedido_productos, asignar gasto proporcional al flete
  const pidProds={};
  mixData.forEach(r=>{
  ...
  // Ordenar por volumen descendente, top 15
  const paretoProds=Object.keys(paretoVolMap)
    .filter(p=>paretoVolMap[p]>0)
    .sort((a,b)=>paretoVolMap[b]-paretoVolMap[a])
    .slice(0,15);

  if(paretoProds.length>0){
    const totalVol=Object.values(paretoVolMap).reduce((s,v)=>s+v,0)||1;
    const totalGasto=Object.values(paretoGastoMap).reduce((s,v)=>s+v,0)||1;

    const volVals=paretoProds.map(p=>Math.round(paretoVolMap[p]||0));
    const gastoVals=paretoProds.map(p=>Math.round(paretoGastoMap[p]||0));

    // Línea acumulada % volumen
    let accVol=0;
    const lineaVolAcum=volVals.map(v=>{accVol+=v;return Math.round(accVol/totalVol*100);});
    let accGasto=0;
    const lineaGastoAcum=gastoVals.map(v=>{accGasto+=v;return Math.round(accGasto/totalGasto*100);});

    // Labels cortos (primeras 20 chars)
    const labels=paretoProds.map(p=>p.length>22?p.substring(0,20)+'…':p);

    mkChart('c-pareto-vol',{
      type:'bar',
      data:{labels,datasets:[
        {type:'bar',label:'Cajas',data:volVals,backgroundColor:COLORS.blue,borderRadius:4,borderSkipped:false,yAxisID:'y-vol',order:2},
        {type:'line',label:'% acumulado',data:lineaVolAcum,borderColor:COLORS.red,borderWidth:2.5,pointRadius:4,pointBackgroundColor:COLORS.red,fill:false,tension:0,yAxisID:'y-pct',order:1}
      ]},
      options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},
        plugins:{
          legend:{position:'bottom',labels:{color:textC,font:{size:11},boxWidth:12,padding:10}},
          tooltip:{callbacks:{
            label:c=>c.dataset.label==='Cajas'?`Cajas: ${c.raw.toLocaleString('es-AR')}`:`Acumulado: ${c.raw}%`
          }},
          annotation:{annotations:{line80:{type:'line',yScaleID:'y-pct',yMin:80,yMax:80,borderColor:'rgba(220,38,38,0.4)',borderWidth:1.5,borderDash:[5,5]}}}
        },
        scales:{
          x:{grid:{display:false},ticks:{color:textC,font:{size:10},maxRotation:35}},
          'y-vol':{type:'linear',position:'left',grid:{color:gridC},ticks:{color:COLORS.blue,font:{size:11}},title:{display:true,text:'Cajas',color:COLORS.blue,font:{size:11}}},
          'y-pct':{type:'linear',position:'right',grid:{display:false},min:0,max:100,ticks:{color:COLORS.red,font:{size:11},callback:v=>v+'%'},title:{display:true,text:'% acumulado',color:COLORS.red,font:{size:11}}}
        }}
    });
    ...
    mkChart('c-pareto-gasto',{
      type:'bar',
      data:{labels,datasets:[
        {type:'bar',label:'Gasto logístico',data:gastoVals,backgroundColor:COLORS.amber,borderRadius:4,borderSkipped:false,yAxisID:'y-gasto',order:2},
        {type:'line',label:'% acumulado',data:lineaGastoAcum,borderColor:COLORS.red,borderWidth:2.5,pointRadius:4,pointBackgroundColor:COLORS.red,fill:false,tension:0,yAxisID:'y-pct',order:1}
      ]},
      options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},
        plugins:{
          legend:{position:'bottom',labels:{color:textC,font:{size:11},boxWidth:12,padding:10}},
          tooltip:{callbacks:{
            label:c=>c.dataset.label==='Gasto logístico'?`Gasto: $${Math.round(c.raw).toLocaleString('es-AR')}`:`Acumulado: ${c.raw}%`
          }}
        },
        scales:{
          x:{grid:{display:false},ticks:{color:textC,font:{size:10},maxRotation:35}},
          'y-gasto':{type:'linear',position:'left',grid:{color:gridC},ticks:{color:COLORS.amber,font:{size:11},callback:v=>'$'+Math.round(v/1000)+'k'},title:{display:true,text:'Gasto logístico',color:COLORS.amber,font:{size:11}}},
          'y-pct':{type:'linear',position:'right',grid:{display:false},min:0,max:100,ticks:{color:COLORS.red,font:{size:11},callback:v=>v+'%'},title:{display:true,text:'% acumulado',color:COLORS.red,font:{size:11}}}
        }}
    });

    // Calcular cuántos productos = 80% del volumen
    const prod80vol=lineaVolAcum.findIndex(v=>v>=80)+1;
    const prod80gasto=lineaGastoAcum.findIndex(v=>v>=80)+1;
    const insightEl=document.getElementById('pareto-insight');
    if(insightEl) insightEl.innerHTML=`
      <span style="margin-right:20px">📦 <strong>${prod80vol} producto${prod80vol>1?'s':''}</strong> generan el 80% del volumen</span>
      <span>💸 <strong>${prod80gasto} producto${prod80gasto>1?'s':''}</strong> generan el 80% del gasto logístico</span>
    `;
  }
...
function calcSim(){
  ...
  if(totalCajas<=0||totalValorLista===0){empty.style.display='block';content.style.display='none';return;}
  empty.style.display='none';content.style.display='block';

  const cajasFlete=Math.ceil(totalCajas);
  const pallets=cajasFlete>=31?Math.ceil(cajasFlete/31):0;
  const flete=calcFlete(region,cajasFlete,pallets)||0;
  ...
  const costoCaja=cajasFlete>0?totalLog/cajasFlete:totalLog;
  const pctV=pctFinal;
  ...
  el('sim-total-cajas').textContent=totalUnidades+' u · '+cajasFlete+' caja'+(cajasFlete>1?'s':'');
...
function renderReglasVenta(){
  ...
  cont.innerHTML=`
    <!-- Header con contexto -->
    <div style="background:var(--blue-light,rgba(37,99,235,0.07));border:1px solid rgba(37,99,235,0.15);border-radius:var(--radius-lg);padding:16px 20px;margin-bottom:20px;display:flex;gap:16px;align-items:center;flex-wrap:wrap">
