// ============================================================
// DASHBOARD.JS — v2.0
// Fixes: orden meses, filtro reactivo, simulador mejorado,
// KPIs Supply Chain, mínimos por zona en simulador
// ============================================================
...
// FIX 5: Ordenar meses cronológicamente
function sortMeses(meses) {
  ...
  return meses.sort((a, b) => {
    const [mA, yA] = a.toLowerCase().split(' de ').length > 1 ? a.toLowerCase().split(' de ') : a.toLowerCase().split(' ');
    const [mB, yB] = b.toLowerCase().split(' de ').length > 1 ? b.toLowerCase().split(' de ') : b.toLowerCase().split(' ');
...
function renderChartsCostos() {
  ...
  // FIX 5: orden cronológico acumulado
  const mesMap={};
  rows.forEach(r=>{if(r.mes)mesMap[r.mes]=(mesMap[r.mes]||0)+(r.total||0);});
  const mL=sortMeses(Object.keys(mesMap));
  let acc=0;const accData=mL.map(m=>{acc+=mesMap[m]||0;return Math.round(acc);});
...
function renderChartsServicio() {
  ...
  // Costo logístico / Facturación (% logístico promedio ponderado)
  ...
  // Pedidos despachados en el día (dias_prep <= 1)
  ...
  // KPIs principales de servicio
  document.getElementById('kpi-servicio').innerHTML=`
    <div class="kpi-card accent-green"><div class="label">OTIF Klozer</div><div class="value" style="color:var(--green)">${otif.toFixed(1)}%</div><div class="sub">Meta &gt;85% · Entrega completa y a tiempo</div></div>
    ...
    <div class="kpi-card accent ${diasPrepAvg<=1?'accent-green':'accent-amber'}"><div class="label">Prep. interna prom.</div><div class="value" style="color:var(--${diasPrepAvg<=1?'green':'amber'})">${diasPrepAvg.toFixed(1)} d</div><div class="sub">Meta &lt;1 día · Confirmación → salida</div></div>
    <div class="kpi-card ${pctDespEnDia>=90?'accent-green':'accent-amber'}"><div class="label">Desp. en el día</div><div class="value" style="color:var(--${pctDespEnDia>=90?'green':'amber'})">${pctDespEnDia.toFixed(1)}%</div><div class="sub">Meta &gt;90% · Agilidad operativa</div></div>
    <div class="kpi-card ${tasaError<2?'accent-green':'accent-red'}"><div class="label">Tasa de error</div><div class="value" style="color:var(--${tasaError<2?'green':'red'})">${tasaError.toFixed(1)}%</div><div class="sub">Meta &lt;2% · Inc. + devueltos + eliminados</div></div>
    <div class="kpi-card ${costoSobreFact<10?'accent-green':'accent-red'}"><div class="label">Costo log. / Fact.</div><div class="value" style="color:var(--${costoSobreFact<10?'green':'red'})">${costoSobreFact.toFixed(1)}%</div><div class="sub">Meta &lt;8-10% · Peso logístico</div></div>
    <div class="kpi-card ${devueltos===0?'':'accent-red'}"><div class="label">Devueltos</div><div class="value ${devueltos>0?'red':''}">${devueltos}</div><div class="sub">Meta &lt;1% devoluciones</div></div>
    <div class="kpi-card"><div class="label">Fill Rate</div><div class="value" style="color:var(--text3)">Sin datos</div><div class="sub">Requiere unidades pedidas vs entregadas</div></div>
  `;

  // OTIF por mes (orden cronológico)
  const mesOtif={};
...
function calcSim() {
  ...
  // Calcular totales desde productos individuales
  let totalCajas = 0;
  let totalValor = 0;
  let productosUsados = [];
  ...
  // También permitir valor manual si no hay productos
  ...
  if (cajasManual > 0 && valorManual > 0) {
    totalCajas += cajasManual;
    totalValor += valorManual;
  }

  if (totalCajas === 0 || totalValor === 0) {
  ...
  if (productosUsados.length > 0) {
    detEl.textContent = productosUsados.join(' · ');
  ...
  if (pctL >= 8) {
    for (let c=totalCajas+1; c<=50; c++) {
      const p2 = totalCajas > 0 ? totalValor/totalCajas : 0;
      const extraVal = (c - totalCajas) * p2;
      ...
      if (pct2 < 8) { msg = `Agregando ${c-totalCajas} cajas más (total ${c}) el pedido quedaría en ${pct2.toFixed(1)}% logístico.`; break; }
    }
    if (!msg) msg = 'Para bajar al 8%, aumentá el valor de la mercadería o consolidá con otro pedido al mismo destino.';
  } else {
    msg = `Este pedido es rentable con ${totalCajas} caja${totalCajas>1?'s':''}.`;
  }
  document.getElementById('sim-threshold-msg').textContent = msg;
}

// ---- TABLA MÍNIMOS POR ZONA (FIX 1) ----
function renderSimThresh() {
  ...
  el.innerHTML = zones.map(z => {
    const mins = prods.map(p => {
      for (let c=1; c<=31; c++) {
        const u = getTarifa(z,c);
        if (u && p>0 && (u/p)<0.08) return c;
      }
      return '>30';
    });
