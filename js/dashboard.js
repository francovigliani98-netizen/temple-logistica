...
function renderChartsCostos(){
  ...
  // ---- MIX DE PRODUCTOS (movido de Resumen) ----
  ...
  mixFilt.forEach(r=>{
    ...
    if(!mixMap[r.mes][r.producto])mixMap[r.mes][r.producto]={cajas:0};
    mixMap[r.mes][r.producto].cajas+=(parseFloat(r.cantidad)||0);
  });
  const mixMeses=sortMeses(Object.keys(mixMap));
  const prodTotalsMix={};
  mixFilt.forEach(r=>{if(r.producto)prodTotalsMix[r.producto]=(prodTotalsMix[r.producto]||0)+(parseFloat(r.cantidad)||0);});
  const mixProds=Object.keys(prodTotalsMix).sort((a,b)=>prodTotalsMix[b]-prodTotalsMix[a]);
  ...
  if(selProd&&selProd.options.length<=1){
    mixProds.forEach(p=>{const o=document.createElement('option');o.value=p;o.textContent=p;selProd.appendChild(o);});
    if(mixProdFiltro)selProd.value=mixProdFiltro;
  }
  if(mixMeses.length>0&&mixProds.length>0){
    let datasets;
    if(mixProdFiltro){
      datasets=[{type:'line',label:mixProdFiltro,
        data:mixMeses.map(m=>Math.round(mixMap[m]?.[mixProdFiltro]?.cajas||0)),
        borderColor:COLORS.blue,backgroundColor:'rgba(37,99,235,0.1)',fill:true,tension:0.3,
        pointRadius:6,pointBackgroundColor:COLORS.blue,pointBorderColor:'#fff',pointBorderWidth:2,borderWidth:2.5}];
    }else{
      datasets=mixProds.map((prod,i)=>({label:prod,
        data:mixMeses.map(m=>Math.round(mixMap[m]?.[prod]?.cajas||0)),
        backgroundColor:mixColors[i%mixColors.length],borderRadius:i===0?4:0,borderSkipped:false,stack:'mix'}));
    }
    mkChart('c-mix-productos',{type:'bar',data:{labels:mixMeses,datasets},
      options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},
        plugins:{legend:{position:'bottom',labels:{color:textC,font:{size:11},boxWidth:12,padding:10}},
          tooltip:{callbacks:{
            label:c=>{
              const val=c.raw,mes=c.label;
              const totalMes=mixProds.reduce((s,p)=>s+(mixMap[mes]?.[p]?.cajas||0),0);
              const pct=totalMes>0?Math.round(val/totalMes*100):0;
              return `${c.dataset.label}: ${val} cajas (${pct}%)`;
            },
            footer:items=>{const tot=items.reduce((s,i)=>s+(i.raw||0),0);return `Total: ${tot} cajas`;}
          }}},
        scales:{
...
function calcSim(){
  ...
  let msg='';
  if(pctV>=8){
    for(let c=cajasFlete+1;c<=cajasFlete+30;c++){
      const vpC=cajasFlete>0?totalValorFinal/cajasFlete:0;
      const eV=(c-cajasFlete)*vpC;
      const f2=calcFlete(region,c,c>=31?Math.ceil(c/31):0)||0;
      const t2=f2+(totalValorFinal+eV)*0.012;
      const p2=t2/(totalValorFinal+eV)*100;
      if(p2<8){msg='Agregando '+(c-cajasFlete)+' cajas más (total '+c+') quedaría en '+p2.toFixed(1)+'%.';break;}
    }
    if(!msg)msg='Para bajar al 8%, aumentá el valor o reducí los descuentos.';
  }else msg='Este pedido es rentable.';
