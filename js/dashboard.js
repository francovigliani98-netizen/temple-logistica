// ============================================================
// DASHBOARD.JS — v3.1
// Novedades: buscador de código postal → zona automática
// en el Simulador (consulta tabla zonas_cp en Supabase)
// ============================================================
...
async function loadTarifario() {
  try {
    ...
    if(error || !data || !data.length) return; // usar TARIFF hardcodeado
    const nuevo = {};
    data.forEach(r => {
      nuevo[r.zona] = [r.p1, r.p2, r.p3, r.p4, r.p5, r.p6, r.p7, r.p8];
    });
    if(Object.keys(nuevo).length > 0) TARIFF = nuevo;
  } catch(e) { /* fallback al TARIFF hardcodeado */ }
}
...
// Mapeo: zonas Klozer (tabla zonas_cp) → zonas TARIFF del dashboard
const ZONA_MAP = {
...
async function buscarCP() {
  ...
  try {
    ...
    // Completar el select de zona y disparar el cálculo
