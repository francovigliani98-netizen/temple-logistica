...
function calcSim(){
  ...
  // Recolectar productos con cantidad y descuento individual
  let totalCajas=0, totalValorLista=0, totalValorFinal=0;
  const productosUsados=[];

  const descKeys={ // mapeo qty key -> desc key
    'sim-qty-cerveza':'sim-desc-cerveza',
    'sim-qty-vermu':'sim-desc-vermu',
    'sim-qty-gin500':'sim-desc-gin500',
    'sim-qty-gin750':'sim-desc-gin750',
    'sim-qty-alta':'sim-desc-alta'
  };

  for(const[nombre,prod]of Object.entries(PRODUCTS)){
    const qty=parseInt(document.getElementById(prod.key)?.value)||0;
    if(qty>0){
      const descPct=parseFloat(document.getElementById(descKeys[prod.key])?.value)||0;
      const descClamp=Math.max(0,Math.min(100,descPct));
      const valorLista=qty*prod.price;
      const valorFinal=valorLista*(1-descClamp/100);
      totalCajas+=qty;
      totalValorLista+=valorLista;
      totalValorFinal+=valorFinal;
      const descLabel=descClamp>0?` (-${descClamp}%)`:'';
      productosUsados.push(`${qty} ${nombre}${descLabel}`);
    }
  }

  // Producto manual (sin descuento)
  ...
  if(cajasManual>0&&valorManual>0){
    ...
    totalValorLista+=valorManual;
    totalValorFinal+=valorManual;
  }
  ...
  const seguro=totalValorFinal*0.012; // seguro sobre valor real (con desc)
  const totalLog=flete+seguro;

  // Dos cifras de % logístico
  ...
  // El veredicto se basa en el % logístico SOBRE PRECIO CON DESCUENTO (caso peor)
  const pctVeredicto=pctFinal;
  const box=document.getElementById('sim-verdict-box');
  box.className='result-big '+(pctVeredicto<8?'green':pctVeredicto<15?'amber':'red');
  const colorL=pctLista<8?COLORS.green:pctLista<15?COLORS.amber:COLORS.red;
  const colorF=pctFinal<8?COLORS.green:pctFinal<15?COLORS.amber:COLORS.red;
  document.getElementById('sim-verdict-text').textContent=pctVeredicto<8?'Pedido rentable':pctVeredicto<15?'Margen ajustado':'Pedido no rentable';
  document.getElementById('sim-verdict-text').style.color=pctVeredicto<8?COLORS.green:pctVeredicto<15?COLORS.amber:COLORS.red;
  document.getElementById('sim-pct-val').textContent=pctLista.toFixed(1)+'%';
  document.getElementById('sim-pct-val').style.color=colorL;
  document.getElementById('sim-pct-val-desc').textContent=pctFinal.toFixed(1)+'%';
  document.getElementById('sim-pct-val-desc').style.color=colorF;
  ...
  document.getElementById('sim-valor-mercaderia').textContent=peso(totalValorLista);

  // Mostrar filas de descuento si aplica
  const descuentoTotal=totalValorLista-totalValorFinal;
  const rowDesc=document.getElementById('sim-row-descuento');
  const rowFinal=document.getElementById('sim-row-valor-final');
  if(descuentoTotal>0){
    rowDesc.style.display='flex';
    rowFinal.style.display='flex';
    document.getElementById('sim-descuento-total').textContent='-'+peso(descuentoTotal);
    document.getElementById('sim-valor-final').textContent=peso(totalValorFinal);
  } else {
  ...
  if(pctVeredicto>=8){
    for(let c=totalCajas+1;c<=50;c++){
      const pUnit=totalCajas>0?totalValorFinal/totalCajas:0;
      const extraVal=(c-totalCajas)*pUnit;
      const f2=calcFlete(region,c,c>=31?Math.ceil(c/31):0)||0;
      const tot2=f2+(totalValorFinal+extraVal)*0.012;
      const pct2=tot2/(totalValorFinal+extraVal)*100;
      if(pct2<8){msg=`Agregando ${c-totalCajas} cajas más (total ${c}) quedaría en ${pct2.toFixed(1)}% logístico.`;break;}
    }
    if(!msg)msg='Para bajar al 8%, aumentá el valor de la mercadería o reducí los descuentos.';
