...
function renderReglasVenta(){
  ...
  const valMinimo=flete1>0?flete1/0.068:null; // ~8% - 1.2% seguro
  ...
  cont.innerHTML=`
    ...
    <div style="background:var(--blue-light,rgba(37,99,235,0.07));border:1px solid rgba(37,99,235,0.15);border-radius:var(--radius-lg);padding:16px 20px;margin-bottom:20px;display:flex;gap:16px;align-items:center;flex-wrap:wrap">
      ...
      ${valMinimo?`<div style="flex:1;min-width:200px">
        <div style="font-size:12px;color:var(--text2);margin-bottom:2px">Valor mínimo del pedido (cualquier producto)</div>
        <div style="font-size:18px;font-weight:700;color:var(--blue)">$${Math.ceil(valMinimo/1000)*1000>0?Math.ceil(valMinimo/1000)*1000:'-'}</div>
      </div>`:''}
      ${desc>0?`<div style="padding:8px 14px;background:rgba(217,119,6,0.1);border-radius:var(--radius);font-size:13px;color:var(--amber);font-weight:600">Con ${desc}% descuento aplicado</div>`:''}
    </div>
