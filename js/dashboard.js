// ============================================================
// DASHBOARD.JS — v3.2
// Novedades: gráfico volumen + costo promedio por zona
// ============================================================
...
async function loadTarifario() {
  try {
    ...
    const nuevo = {};
    data.forEach(r => {
      nuevo[r.zona] = [r.p1, r.p2, r.p3, r.p4, r.p5, r.p6, r.p7, r.p8];
    });
...
function renderChartsMain(){
  ...
  mkChart('c-pct-tendencia',{
      ...
      datasets:[
        {
          type:'bar',
          label:'Rojo (>15%)',
          data:mInc.map(m=>mesIncMap[m]?.rojo||0),
        ...
        },
        {
          type:'bar',
          label:'Amarillo (8-15%)',
          data:mInc.map(m=>mesIncMap[m]?.amarillo||0),
        ...
        },
        {
          type:'bar',
          label:'Verde (<8%)',
          data:mInc.map(m=>mesIncMap[m]?.verde||0),
    ...
    options:{
      ...
      plugins:{
        legend:{position:'bottom',labels:{color:textC,font:{size:11},boxWidth:12,padding:12}},
        tooltip:{
          callbacks:{
            label:c=>{
              if(c.dataset.label==='% logístico promedio') return `Promedio: ${c.raw}%`;
              return `${c.dataset.label}: ${c.raw} pedidos`;
            }
          }
        }
      },
      scales:{
        ...
        'y-cnt':{
          ...
          title:{display:true,text:'Pedidos',color:textC,font:{size:11}}
        },
        'y-pct':{
          type:'linear',
          position:'right',
          grid:{display:false},
          ticks:{color:COLORS.navy,font:{size:11},callback:v=>v+'%'},
          title:{display:true,text:'% logístico',color:COLORS.navy,font:{size:11}},
          min:0
        }
      }
    }
...
function renderChartsCostos(){
  ...
  // ---- NUEVO: Volumen de entregas + costo promedio por zona ----
  const zonaData={};
  rows.forEach(r=>{
    if(!r.region) return;
    if(!zonaData[r.region]) zonaData[r.region]={pedidos:0,totalCosto:0};
    zonaData[r.region].pedidos++;
    zonaData[r.region].totalCosto+=(r.total||0);
  });
  // Ordenar por volumen descendente
  const zonasOrdenadas=Object.keys(zonaData).sort((a,b)=>zonaData[b].pedidos-zonaData[a].pedidos);
  const volLabels=zonasOrdenadas;
  const volPedidos=zonasOrdenadas.map(z=>zonaData[z].pedidos);
  const volPromedio=zonasOrdenadas.map(z=>Math.round(zonaData[z].totalCosto/zonaData[z].pedidos));

  mkChart('c-zona-volumen',{
    type:'bar',
    data:{
      labels:volLabels,
      datasets:[
    ...
    options:{
      ...
      plugins:{
        legend:{
          position:'bottom',
          labels:{color:textC,font:{size:11},boxWidth:12,padding:14}
        },
        tooltip:{
          callbacks:{
            label:function(c){
              if(c.dataset.label==='Entregas') return `Entregas: ${c.raw}`;
              return `Costo promedio: $${Math.round(c.raw).toLocaleString('es-AR')}`;
            }
          }
        }
      },
      scales:{
        x:{
          grid:{color:gridC},
          ticks:{color:textC,font:{size:10},maxRotation:20}
        },
        'y-vol':{
          type:'linear',
          position:'left',
          grid:{color:gridC},
          ticks:{color:COLORS.teal,font:{size:11}},
          title:{display:true,text:'Entregas',color:COLORS.teal,font:{size:11}}
        },
        'y-costo':{
          type:'linear',
          position:'right',
          grid:{display:false},
          ticks:{color:COLORS.amber,font:{size:11},callback:v=>'$'+Math.round(v/1000)+'k'},
          title:{display:true,text:'Costo promedio',color:COLORS.amber,font:{size:11}}
        }
      }
    }
  });
  ...
  // ---- SCATTER: Volumen (cajas) vs. Costo por caja ----
  const regionColors={
    'Local - CABA':COLORS.blue,
    'GBA':COLORS.teal,
    'Centro (capitales)':COLORS.green,
    'Centro (interior)':COLORS.amber,
    'Cuyo (capitales)':COLORS.purple,
    'Cuyo (interior)':'#f59e0b',
    'NEA/NOA (capitales)':'#06b6d4',
    'NEA/NOA (interior)':'#8b5cf6',
    'Patagonia (capitales)':COLORS.red,
    'Patagonia (interior)':'#be123c',
    'Santa Cruz y TDF':'#4b5563',
  };
  // Agrupar puntos por región para datasets separados (así la leyenda funciona)
  const scatterByRegion={};
  rows.forEach(r=>{
    if(!r.cajas||!r.total||r.cajas<1) return;
    const costoCaja=Math.round(r.total/r.cajas);
    const region=r.region||'Otra';
    if(!scatterByRegion[region]) scatterByRegion[region]=[];
    scatterByRegion[region].push({
      x:r.cajas,
      y:costoCaja,
      pid:r.pid||'',
      dest:r.razon_social||r.dest||''
    });
  });
  const scatterDatasets=Object.entries(scatterByRegion).map(([region,pts])=>({
    label:region,
    data:pts,
    backgroundColor:(regionColors[region]||COLORS.gray)+'cc',
    pointRadius:5,
    pointHoverRadius:7,
  }));
  mkChart('c-scatter-volumen',{
    type:'scatter',
    ...
    options:{
      ...
      plugins:{
        legend:{
          position:'bottom',
          labels:{color:textC,font:{size:10},boxWidth:10,padding:10,usePointStyle:true}
        },
        tooltip:{
          callbacks:{
            label:c=>{
              const p=c.raw;
              return [`${p.dest||'Pedido '+p.pid}`,`${p.x} cajas · $${p.y.toLocaleString('es-AR')}/cj`];
            }
          }
        }
      },
      scales:{
        x:{
          ...
          grid:{color:gridC},
          ticks:{color:textC,font:{size:11}},
          title:{display:true,text:'Cantidad de cajas',color:textC,font:{size:11}},
          min:0
        },
        y:{
          grid:{color:gridC},
          ticks:{color:textC,font:{size:11},callback:v=>'$'+Math.round(v/1000)+'k'},
          title:{display:true,text:'Costo por caja',color:textC,font:{size:11}},
          min:0
        }
      }
    }
...
async function buscarCP() {
  ...
  try {
    const { data, error } = await supabaseClient
      .from('zonas_cp')
      .select('zona, localidad, provincia')
      .eq('cp', cp)
      .single();

    if (error || !data) {
      badge.textContent = 'CP no encontrado';
      badge.className = 'badge red';
      return;
    }

    const zonaKlozer = data.zona;
    const zonaSelect = ZONA_MAP[zonaKlozer];

    if (!zonaSelect) {
      badge.textContent = `${data.localidad} — zona "${zonaKlozer}" sin tarifa`;
    ...
    if (select) {
      select.value = zonaSelect;
      calcSim();
    }

    badge.textContent = `${data.localidad}, ${data.provincia} → ${zonaKlozer}`;
...
function calcSim(){
  ...
  let totalCajas=0, totalValor=0, totalLitros=0, totalMargenPeso=0;
  const productosUsados=[], lineaProductos=[];
  for(const[nombre,prod]of Object.entries(PRODUCTS)){
    const qty=parseInt(document.getElementById(prod.key)?.value)||0;
    if(qty>0){
      totalCajas+=qty;
      totalValor+=qty*prod.price;
      totalLitros+=qty*(prod.litros||0);
      totalMargenPeso+=qty*prod.price*(prod.margen||0);
      productosUsados.push(`${qty} ${nombre}`);
      lineaProductos.push({nombre,qty,prod});
    }
  }
  ...
  const costoLitro=totalLitros>0?totalLog/totalLitros:null;

  // Margen bruto promedio ponderado
  const margenProm=totalValor>0?totalMargenPeso/totalValor:null;
  // Ganancia neta: margen bruto - costo logístico (ambos sobre el valor)
  const gananciaNeta=margenProm!=null?margenProm*100-pctL:null;
  ...
  // Costo por litro
  const litroEl=document.getElementById('sim-costo-litro');
  if(litroEl) litroEl.textContent=costoLitro?peso(Math.round(costoLitro))+'/L':'—';
  ...
  // ---- BOTÓN DE PÁNICO: análisis por producto ----
  const panicEl=document.getElementById('sim-panic-block');
  if(panicEl && lineaProductos.length>0 && margenProm!=null){
    const alertas=lineaProductos.map(({nombre,qty,prod})=>{
      if(!prod.margen||!qty) return null;
      const valorProd=qty*prod.price;
      const fProd=(totalValor>0?valorProd/totalValor:0)*totalLog;
      const pctProd=valorProd>0?fProd/valorProd*100:0;
      const ganancia=prod.margen*100-pctProd;
      let icon,clase,texto;
      if(ganancia<0){icon='🚨';clase='red';texto=`${nombre}: pérdida operativa (margen ${(prod.margen*100).toFixed(0)}% − log ${pctProd.toFixed(1)}% = ${ganancia.toFixed(1)}%)`;}
      else if(ganancia<10){icon='⚠️';clase='amber';texto=`${nombre}: margen neto ajustado (${ganancia.toFixed(1)}% restante)`;}
      else{icon='✅';clase='green';texto=`${nombre}: OK (${ganancia.toFixed(1)}% margen neto)`;}
      return `<div style="display:flex;gap:8px;align-items:flex-start;padding:6px 0;border-bottom:1px solid var(--border);font-size:12px"><span>${icon}</span><span style="color:var(--${clase}-dark,var(--${clase}))">${texto}</span></div>`;
    }).filter(Boolean);
    panicEl.innerHTML=`<div style="font-size:12px;font-weight:600;color:var(--text2);margin-bottom:6px;text-transform:uppercase;letter-spacing:.04em">Análisis por producto</div>${alertas.join('')}`;
    panicEl.style.display='block';
  } else if(panicEl) panicEl.style.display='none';

  // ---- COMBO ÓPTIMO: sugerencia específica de upselling ----
  const comboEl=document.getElementById('sim-combo-block');
  if(comboEl){
    let comboMsg='';
    if(pctL>=8){
      // Buscar cuántas cajas de cada producto adicional bajarían al 8%
      const sugerencias=[];
      for(const[nombre,prod]of Object.entries(PRODUCTS)){
        for(let extra=1;extra<=10;extra++){
          const cAux=totalCajas+extra;
          const vAux=totalValor+extra*prod.price;
          const fAux=calcFlete(region,cAux,cAux>=31?Math.ceil(cAux/31):0)||0;
          const tAux=fAux+vAux*0.012;
          const pAux=tAux/vAux*100;
          if(pAux<8){
            sugerencias.push({nombre,extra,pctFinal:pAux});
            break;
          }
        }
      }
      sugerencias.sort((a,b)=>a.extra-b.extra);
      if(sugerencias.length>0){
        const s=sugerencias[0];
        comboMsg=`💡 <strong>Combo óptimo:</strong> Sumá ${s.extra} caja${s.extra>1?'s':''} de <strong>${s.nombre}</strong> para bajar al ${s.pctFinal.toFixed(1)}% logístico.`;
        if(sugerencias.length>1){
          const s2=sugerencias[1];
          comboMsg+=` También: +${s2.extra} de <strong>${s2.nombre}</strong> → ${s2.pctFinal.toFixed(1)}%.`;
        }
      } else {
        comboMsg='Para bajar al 8%, consolidá con otro pedido al mismo destino.';
      }
    } else {
      comboMsg=`✅ Pedido rentable con ${totalCajas} caja${totalCajas>1?'s':''}. Margen neto estimado: ${gananciaNeta!=null?gananciaNeta.toFixed(1)+'%':'—'}.`;
    }
    comboEl.innerHTML=comboMsg;
    comboEl.style.display='block';
  }

  // ---- COMPARTIR POR WHATSAPP ----
  const shareEl=document.getElementById('sim-share-btn');
  if(shareEl){
    const destino=region;
    const comboTxt=pctL>=8
      ? (() => {
          for(const[nombre,prod]of Object.entries(PRODUCTS)){
            for(let extra=1;extra<=10;extra++){
              const cAux=totalCajas+extra,vAux=totalValor+extra*prod.price;
              const fAux=calcFlete(region,cAux,cAux>=31?Math.ceil(cAux/31):0)||0;
              const pAux=(fAux+vAux*0.012)/vAux*100;
              if(pAux<8) return `Sugerimos sumar ${extra} caja${extra>1?'s':''} de ${nombre} para optimizar el flete a ${pAux.toFixed(1)}%.`;
            }
          }
          return 'Considerá consolidar con otro pedido para mejorar el costo.';
        })()
      : `El pedido está optimizado. 🎉`;
    const texto=`Hola! Para tu pedido a *${destino}* de *${totalCajas} caja${totalCajas>1?'s':''}* (${productosUsados.join(', ')}), el costo logístico es de *${peso(totalLog)}* (${pctL.toFixed(1)}% del valor). ${comboTxt}`;
    const url=`https://wa.me/?text=${encodeURIComponent(texto)}`;
    shareEl.onclick=()=>window.open(url,'_blank');
    shareEl.style.display='inline-flex';
  }

  // backward compat: threshold-msg (oculto, reemplazado por combo)
  const threshEl=document.getElementById('sim-threshold-msg');
