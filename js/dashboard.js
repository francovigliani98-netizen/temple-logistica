  ...
  dd.innerHTML = filtrados.slice(0, 20).map(p => `
    <div onclick="seleccionarProducto(${p.sku})"
      ...
      <div style="font-size:11px;color:var(--text3);margin-top:2px">
        SKU ${p.sku} · $${Math.round((p.precio_unidad||0) * (p.unidades_por_bulto||1)).toLocaleString('es-AR')}/bulto · ${p.unidades_por_bulto} u/bulto
      </div>
    </div>`).join('');
  dd.style.display = 'block';
}
...
function seleccionarProducto(sku) {
  ...
  const precioBultoReal = Math.round((prod.precio_unidad || 0) * (prod.unidades_por_bulto || 1));
  prod._precioBulto = precioBultoReal;
  document.getElementById('real-prod-precio-base').textContent =
    `SKU ${prod.sku} · Precio lista: $${precioBultoReal.toLocaleString('es-AR')}/bulto (${prod.unidades_por_bulto} u/bulto)`;
...
function actualizarLineaPreview() {
  ...
  const precioBase = prodSeleccionado._precioBulto || Math.round((prodSeleccionado.precio_unidad||0) * (prodSeleccionado.unidades_por_bulto||1));
