...
function renderChartsMain(){
  ...
  // ---- MIX DE PRODUCTOS POR MES (mejorado) ----
  const mesFiltro=document.getElementById('filter-mes').value;
  const mixFilt=mesFiltro?mixData.filter(r=>r.mes===mesFiltro):mixData;

  // Agrupar: mes → producto → {cajas, unidades}
  const mixMap={};
  mixFilt.forEach(r=>{
    if(!r.mes||!r.producto)return;
    if(!mixMap[r.mes])mixMap[r.mes]={};
    if(!mixMap[r.mes][r.producto])mixMap[r.mes][r.producto]={cajas:0,unidades:0};
    const cant=parseFloat(r.cantidad)||0;
    // cantidad en pedido_productos = cajas (bultos). Unidades = cajas × unidades_por_bulto
    // Como no tenemos upb aquí, guardamos cajas y calculamos unidades desde listaPrecios
    mixMap[r.mes][r.producto].cajas+=cant;
  });
  ...
  const mixModo=(document.getElementById('mix-modo')?.value)||'cajas';
  const mixProdFiltro=(document.getElementById('mix-prod-filtro')?.value)||'';

  // Llenar selector de producto si existe y está vacío
  ...
  if(mixMeses.length>0&&mixProds.length>0){
    let datasets;
    if(mixProdFiltro){
      // Modo línea: evolución de un producto específico
      datasets=[{
        type:'line',
        label:mixProdFiltro,
      ...
      // Modo barras apiladas: todos los productos
        ...
        plugins:{
          ...
          tooltip:{callbacks:{
            label:c=>{
              const val=c.raw;
              const mes=c.label;
              // Calcular total del mes para el %
