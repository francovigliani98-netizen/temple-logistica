...
function calcReal() {
  ...
  // Semáforo
  const color = pctL < 8 ? 'var(--green)' : pctL < 15 ? 'var(--amber)' : 'var(--red)';
  ...
  document.getElementById('real-verdict-text').style.color = color;
  document.getElementById('real-pct-val').textContent = pctL.toFixed(1) + '%';
  document.getElementById('real-pct-val').style.color = color;
  ...
  // Margen neto (opcional)
  const margenBox = document.getElementById('real-margen-box');
  if (!isNaN(margenBruto) && margenBruto > 0) {
    const margenNeto = margenBruto - pctL;
    const netoColor = margenNeto > 5 ? 'var(--green)' : margenNeto > 0 ? 'var(--amber)' : 'var(--red)';
    document.getElementById('real-margen-bruto-val').textContent = margenBruto.toFixed(1) + '%';
    document.getElementById('real-pct-log-val').textContent = pctL.toFixed(1) + '%';
    document.getElementById('real-margen-neto-val').textContent = margenNeto.toFixed(1) + '%';
    document.getElementById('real-margen-neto-val').style.color = netoColor;
    margenBox.style.display = 'block';
  } else {
  ...
  if (pctL >= 8) {
    for (let c = cajas + 1; c <= 60; c++) {
      const pUnit = valor / cajas;
      const extraVal = (c - cajas) * pUnit;
      const f2 = calcFlete(region, c, c >= 31 ? Math.ceil(c / 31) : 0) || 0;
      const tot2 = f2 + (valor + extraVal) * 0.012;
      const pct2 = tot2 / (valor + extraVal) * 100;
      if (pct2 < 8) { msg = `Agregando ${c - cajas} cajas más (total ${c}) quedaría en ${pct2.toFixed(1)}% logístico.`; break; }
    }
